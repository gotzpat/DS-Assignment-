...
When evaluating feature importance, it's crucial to consider both statistical significance (p-value) and practical significance (effect size). With large sample sizes, even trivial differences can achieve statistical significance, making effect size interpretation essential.

**Effect Size (η²)** measures the proportion of variance in student outcomes explained by each feature, with typical interpretations:

- **η² = 0.01** (1%): Small effect
- **η² = 0.06** (6%): Medium effect  
- **η² = 0.14** (14%): Large effect

For example, marital status has a highly significant p-value (2.66e-09) but explains less than 1% of variance (η² = 0.009), indicating negligible practical importance. In contrast, "Curricular units 2nd sem (grade)" explains 34% of variance (η² = 0.339), representing a genuinely large and meaningful effect. When identifying important predictors, prioritize features with larger effect sizes rather than relying solely on p-values.
...
### 4.7.4 - Key Findings for Demographic & Socioeconomic Background 
...
display(Markdown("""
The Random Forest classifier achieved an overall accuracy of 67.3%, correctly predicting student outcomes for approximately two-thirds of cases. However, accuracy alone doesn't tell the full story—we need to examine how well the model performs for each class.

**Performance varies significantly across the three outcomes.** The model performs best at identifying Dropout and Graduate students, with F1-scores of 0.707 and 0.773 respectively. However, it struggles with the Enrolled category, achieving only an F1-score of 0.392. This means the model has difficulty distinguishing currently enrolled students from those who will eventually dropout or graduate.

**The confusion matrix reveals specific prediction patterns.** The model correctly identifies 185 out of 284 dropout cases (65.1% recall) and 338 out of 442 graduates (76.5% recall). The challenge with Enrolled students is clear: only 73 out of 159 (45.9%) are correctly classified, with many being misclassified as either future graduates (53 cases) or potential dropouts (33 cases). This reflects the inherent difficulty in predicting outcomes for students still in progress—their final status remains uncertain until they either complete or leave their program.

**Handling class imbalance.** With fewer Enrolled students in the dataset (159 vs 284 Dropouts and 442 Graduates), the model has less training data to learn patterns for this group. To address this imbalance, we used the `class_weight='balanced'` parameter in the Random Forest model, which automatically adjusts weights inversely proportional to class frequencies. This ensures the model pays equal attention to all three classes during training rather than being biased toward the majority class. While this helps mitigate the imbalance, the fundamental challenge remains: enrolled students represent an "in-between" state that's harder to characterize than the definitive outcomes of dropout or graduation.

**Predicted distribution.** The last figure shows the predicted distribution of classes after applying the model. Despite the class imbalance in the original data, the model with balanced class weights manages to capture the real distribution reasonably well, demonstrating that the balancing strategy is effective.
"""))
...
display(Markdown("""
Random Forest models calculate feature importance by measuring how much each feature contributes to reducing prediction error across all decision trees in the forest. Features that consistently lead to better splits and more accurate predictions receive higher importance scores. Unlike the ANOVA analysis which examines relationships between individual features and outcomes, Random Forest importance reflects each feature's predictive value within the context of all other features—capturing complex interactions and non-linear relationships that make features useful for classification.
"""))
...
display(Markdown("""
Explain how these are different from ANOVA, explain a bit why could these be important
"""))
...
display(Markdown("""
LIME (Local Interpretable Model-agnostic Explanations) helps us understand why the model made a specific prediction for an individual student. Unlike feature importance which shows what matters globally across all predictions, LIME reveals which features drove the decision for this particular case.
"""))
...
display(Markdown("""The table shows the top 10 features that most influenced this prediction. Positive weights push the prediction toward the predicted class, while negative weights push against it. The magnitude indicates the strength of influence. For example, if a student has a low 2nd semester grade (feature 0), this strongly increases their dropout probability, while being older than 25 (feature 2, negative weight) actually decreases dropout risk for this individual—even though the model still predicts dropout overall due to the combined effect of all features. CONTINUE HERE
"""))
# ============================================================================
...
display(Markdown("*Analyzing 100 samples per class to identify robust patterns*\n"))
...
display(Markdown(""" Here Explain the difference between the most important features of the different classes"""))
...
display(Markdown("""Again talk about the different features."""))
# ============================================================================
...
display(Markdown("""Again talk about the different features but now globally."""))
...
display(Markdown("""Compare also with ANOVA."""))
# ============================================================================
# KEY INSIGHTS
# ============================================================================
