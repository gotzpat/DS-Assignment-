...
display(Markdown("\n### 5.2.1-Top 15 Most Important Features (Random Forest)\n"))
...
display(Markdown("\n## 5.3-LIME Explanations - Individual Examples\n"))
...
class_interpretations = {
    'Dropout': """
**Interpretation:** This student was correctly identified as at risk of dropping out. The dominant factor is their poor academic performance, with a second semester grade of 10.78 or below strongly pushing toward dropout (large negative weight). Additionally, lacking scholarship support increases dropout risk. Protective factors like not being in debt, younger age (≤19), and decent first semester grades (≤11) provide some counterbalance, but are insufficient to overcome the poor second semester performance. This highlights how academic struggles, especially in later semesters, are critical indicators of dropout risk.
""",
    'Enrolled': """
...
```{python}
display(Markdown("\n## 5.7-Feature Importance Comparison: Random Forest, LIME, and ANOVA\n"))
...
## Appendix
Analyzing 100 samples per class to identify robust patterns and handle class imbalance. Why we put it in appendix?

```{python}

display(Markdown("\n## 5.4-LIME Analysis - Class-Wise Aggregation\n"))
...
for class_label in sorted(y.unique()):
    ...
    if class_label == "Dropout":
        display(Markdown(f"\n### 5.4.1-Top 15 Features for Predicting: **{class_label}**\n"))
...
display(Markdown("\n## 5.5-Feature Importance Comparison Across Classes\n"))
